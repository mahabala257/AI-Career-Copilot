...
async def resume_agent_node(state: CareerCopilotState) -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"[ResumeAgent] Sending request to AI service")
        ...
        logger.debug(f"[ResumeAgent] AI service raw response ({len(raw)} chars): {raw[:200]}")

    except Exception as e:
        logger.error(f"[ResumeAgent] AI service call failed: {e}", exc_info=True)
        return {
            "resume_analysis": _empty_analysis(
                reason=f"AI analysis temporarily unavailable. Please try again.",
                target_role=target_role,
            ),
            "agents_called": [AgentName.RESUME],
