...
def build_company_research_prompt(
    ...
    filters = []
    if work_mode and work_mode.lower() not in ("any", ""):
        filters.append(f"Work Mode Preference: {work_mode}")
    if employment_type and employment_type.lower() not in ("any", ""):
        filters.append(f"Employment Type Preference: {employment_type}")
    filters_section = ("\n".join(filters) + "\n") if filters else ""
...
{filters_section}{rag_section}
Provide a complete preparation guide. For typical_openings, list 4-6 real roles this
company commonly hires for. Each opening must reflect the candidate's work_mode and
employment_type preferences where specified (otherwise list a variety).
Use the knowledge base information as the primary source. Supplement with your knowledge
where the knowledge base is incomplete.
