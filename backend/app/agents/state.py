...
class CareerCopilotState(TypedDict, total=False):
    ...
    # ── 5. Skill Gap Agent Inputs & Outputs ───────────────────────────────────
    current_skills: list[str]          # User's self-reported current skills
    generate_learning_path: bool       # Whether to generate per-skill learning paths
    generate_roadmap: bool             # Whether to generate a timeline roadmap
    ...
    employment_type: str      # "Full-time" | "Part-time" | "Contract" | "Internship" | "Any"
...
def create_initial_state(
    ...
    return CareerCopilotState(
        ...
        error=None,
        error_agent=None,
        # Skill gap agent inputs — populated by skill gap service before invoking
        current_skills=[],
