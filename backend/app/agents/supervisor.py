...
async def supervisor_node(state: CareerCopilotState) -> dict[str, Any]:
    ...
    # Short-circuit: if the caller pre-set next_agent, skip the LLM routing call.
    # This saves tokens and avoids rate limits for dedicated API endpoints (e.g.
    # /api/skills/analyze, /api/interview/generate) that already know their target.
    if state.get("next_agent"):
        logger.info(
            f"[Supervisor] next_agent pre-set to '{state['next_agent']}' — skipping LLM routing"
        )
        return {
            "next_agent": state["next_agent"],
            "agent_queue": state.get("agent_queue", []),
            "is_multi_agent": bool(state.get("agent_queue")),
            "routing_reasoning": "pre-set by caller",
