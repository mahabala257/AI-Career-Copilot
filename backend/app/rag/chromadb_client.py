...
class GeminiEmbeddingFunction:
    ...
    def embed_query(self, text: str = None, **kwargs) -> list[float]:
        ...
        t = text if text is not None else kwargs.get("input", "")
        if isinstance(t, list):
            t = t[0] if t else ""
        return self._model.embed_query(t)
...
class FallbackEmbeddingFunction:
    ...
    def embed_query(self, text: str = None, **kwargs) -> list[float]:
        ...
        if not self._use_fallback:
            try:
                return self._primary.embed_query(t)
            except Exception as e:
                logger.warning(
                ...
                self._use_fallback = True
        return self._fallback.embed_query(t)


@lru_cache(maxsize=1)
def _get_default_local_ef():
    """
    ChromaDB's built-in DefaultEmbeddingFunction — all-MiniLM-L6-v2 (384-dim)
    running locally via ONNX runtime. No API key required, works fully offline
    after a one-time ~80 MB model download (cached under ~/.cache/chroma).

    This is the correct default for this project: the Gemini "AQ." key cannot
    perform embeddings (text-embedding-004 returns 404 for that key format),
    and the old hash fallback produced non-semantic vectors AND crashed
    ChromaDB 1.5.9 query validation. MiniLM gives real semantic retrieval and
    is fully compatible with the ChromaDB EmbeddingFunction interface.
    """
    from chromadb.utils import embedding_functions
    logger.info("[ChromaDB] Using local DefaultEmbeddingFunction (all-MiniLM-L6-v2, 384-dim, offline)")
    return embedding_functions.DefaultEmbeddingFunction()


def get_embedding_function():
    """
    Returns the embedding function used for all collections.

    Priority order:
      1. OpenAI (if OPENAI_API_KEY set) — high-quality hosted embeddings.
      2. Local MiniLM via ChromaDB DefaultEmbeddingFunction — offline, no key,
         real semantic search. This is the default for local development.

    NOTE: Gemini embeddings are intentionally NOT used by default because the
    provisioned "AQ." Google key cannot call the embedding endpoint. The
    GeminiEmbeddingFunction class is retained for a future valid key; to use it,
    wrap it with FallbackEmbeddingFunction here.
    """
    if settings.openai_api_key:
        try:
            return OpenAIEmbeddingFunction()
        except Exception as e:
            logger.warning(f"[ChromaDB] OpenAI EF init failed ({e}); using local MiniLM")
    return _get_default_local_ef()
