...
async def run_company_research(
    *,
    ...
    company_name = sanitize_user_text(company_name, max_len=100)
    ...
    initial_state["next_agent"]  = AgentName.COMPANY_RESEARCH  # type: ignore[index]
...
async def get_company_history(user_id: str, db: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    rows = await db.execute(
        ...
        .where(CompanyResearch.user_id == uuid.UUID(user_id))
