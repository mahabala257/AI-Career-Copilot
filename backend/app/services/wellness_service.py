...
async def run_wellness_checkin(
    *,
    ...
    mood_message = sanitize_user_text(mood_message, max_len=2000)
    ...
    initial_state["next_agent"]  = AgentName.WELLNESS  # type: ignore[index]
...
async def get_wellness_history(user_id: str, db: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    rows = await db.execute(
        ...
        .where(WellnessCheckin.user_id == uuid.UUID(user_id))
