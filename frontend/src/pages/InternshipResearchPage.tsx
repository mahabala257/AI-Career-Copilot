...
export default function InternshipResearchPage() {
  ...
  const [result, setResult] = usePersistentState<InternshipResearchResponse | null>("internship-result", null);
  ...
  const mutation = useMutation({
    mutationFn: () =>
      internshipApi.research({
        ...
        duration_months: parseInt(durationMonths) || undefined,
      }),
    onSuccess: (data) => setResult(data),
  });

  const r = result?.result;
  const timelineEntries = r?.application_timeline ? Object.entries(r.application_timeline) : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {!r ? (
          ...
          ) : (r.error_reason || !r.recommended_companies?.length) ? (
            <Card className="h-48 flex items-center justify-center border-amber-200 bg-amber-50">
              <div className="text-center text-amber-700 space-y-3 px-6">
                ...
                <p className="text-sm">
                  Couldn&apos;t generate internship recommendations{r.error_reason ? ` (${r.error_reason})` : ""}. Please try again in a moment.
                </p>
                  ...
                  {r.recommended_companies.map((c, i) => (
                    <Card key={i}>
                      ...
                      <CardContent className="space-y-2 text-sm">
                        ...
                        {(c.selection_process?.length ?? 0) > 0 && (
