...
function ProjectCard({ project }: { project: RecommendedProject }) {
  ...
  const ts = project.tech_stack || ({} as RecommendedProject["tech_stack"]);
  const allTech = [
    ...(ts.backend || []), ...(ts.frontend || []), ...(ts.ai_ml || []),
    ...(ts.database || []), ...(ts.devops || []),
  ];
...
export default function ProjectRecommenderPage() {
  ...
  const [result, setResult] = usePersistentState<ProjectRecommendResponse | null>("projects-result", null);
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {!r ? (
          ...
          ) : (r.error_reason || !r.recommended_projects?.length) ? (
            <Card className="h-48 flex items-center justify-center border-amber-200 bg-amber-50">
              <div className="text-center text-amber-700 space-y-3 px-6">
                ...
                <p className="text-sm">
                  Couldn&apos;t generate project ideas{r.error_reason ? ` (${r.error_reason})` : ""}. Please try again in a moment.
                </p>
            ...
            <>
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  {(r.portfolio_action_plan?.length ?? 0) > 0 && (
              ...
              {/* Projects to avoid */}
              {(r.projects_to_avoid?.length ?? 0) > 0 && (
