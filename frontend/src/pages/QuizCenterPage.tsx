...
export default function QuizCenterPage() {
  ...
  const [quiz, setQuiz] = usePersistentState<QuizGenerateResponse | null>("quiz-quiz", null);
  const [answers, setAnswers] = useState<Record<number, string>>({});
  const [scoreResult, setScoreResult] = usePersistentState<QuizScoreResponse | null>("quiz-score", null);
      ...
      {generateMutation.isError && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="pt-6 text-sm text-red-700">
            Could not generate a quiz right now. Please try again in a moment.
          </CardContent>
        </Card>
      )}

      {/* AI returned an empty quiz */}
      {quiz && !scoreResult && totalQ === 0 && (
        <Card>
          <CardContent className="pt-6 text-center space-y-3">
            <Brain className="w-10 h-10 mx-auto text-muted-foreground/30" />
            <p className="text-muted-foreground text-sm">
              The AI couldn't generate questions{quiz.agent_error ? ` (${quiz.agent_error})` : ""}. Try a different topic or generate again.
            </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Progress value={totalQ ? (answeredCount / totalQ) * 100 : 0} className="w-32 h-2" />
