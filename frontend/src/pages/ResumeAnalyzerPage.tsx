...
export default function ResumeAnalyzerPage() {
  ...
  const [result, setResult] = usePersistentState<ResumeAnalysisResponse | null>("resume-result", null);
  ...
  // The /resume/{id} detail endpoint returns a FLAT record; the results panel
  // expects the nested `analysis` shape. Normalize it when loading from history.
  const loadFromHistory = async (id: string) => {
    const d: any = await resumeApi.getById(id);
    setResult({
      ...
      parse_warnings: [],
      agent_error: null,
      analyzed_at: d.analyzed_at ?? d.created_at,
    });
  };
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {history.items.slice(0, 4).map((h: any) => (
                  <div key={h.resume_id} className="flex items-center justify-between p-2 rounded hover:bg-muted/50 cursor-pointer" onClick={() => loadFromHistory(h.resume_id)}>
                    <div>
                ...
                <CardContent className="p-0">
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 p-4 border-l space-y-2">
                      {result.analysis?.target_role && <p className="text-sm font-medium">For role: {result.analysis.target_role}</p>}
                      {result.page_count > 0 && <p className="text-xs text-muted-foreground">{result.page_count} pages · {result.word_count} words</p>}
