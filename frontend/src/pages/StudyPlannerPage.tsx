...
export default function StudyPlannerPage() {
  ...
  const [plan, setPlan] = usePersistentState<PlanResponse | null>("planner-plan", null);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Error banner if the agent failed */}
      {mutation.isError && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="pt-6 text-sm text-red-700">
            Could not generate a study plan right now. Please try again in a moment.
          </CardContent>
        </Card>
      )}
      ...
      {plan && (() => {
        ...
        const hasContent =
          (plan.plan_type === "daily"   && (pd.sessions?.length ?? 0) > 0) ||
          (plan.plan_type === "weekly"  && (pd.days?.length ?? 0) > 0) ||
          (plan.plan_type === "monthly" && (pd.weeks?.length ?? 0) > 0);
          ...
          <div className="space-y-4">
            ...
            {plan.agent_error && (
              <Card className="border-amber-200 bg-amber-50">
                <CardContent className="pt-4 pb-4 text-sm text-amber-700">
                  The plan was generated with limited detail. {plan.agent_error}
                </CardContent>
              </Card>
            )}
            ...
            {!hasContent ? (
              <Card>
                <CardContent className="pt-6 text-center space-y-3">
                  ...
                  <p className="text-muted-foreground text-sm">
                    The AI returned an empty plan. Set a target role and try generating again.
                  </p>
              ...
              <>
                {plan.plan_type === "daily"   && <DailyPlan   plan={pd} />}
                {plan.plan_type === "weekly"  && <WeeklyPlan  plan={pd} />}
                {plan.plan_type === "monthly" && <MonthlyPlan plan={pd} />}
              </>
            )}
          </div>
        );
