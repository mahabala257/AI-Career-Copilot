...
export const progressApi = {
  ...
  downloadReport: () => api.get("/api/progress/report", { responseType: "blob" }).then((r) => r.data as Blob),
};
...
export const englishApi = {
  ...
  ask: (d: { question: string; history?: { role: string; content: string }[] }) =>
    api.post("/api/english/ask", d, { timeout: 120000 }).then((r) => r.data),
};

export const companyApi = {
  research: (d: { company_name: string; target_role?: string; work_mode?: string; employment_type?: string }) =>
    api.post("/api/company/research", d).then((r) => r.data),
  history: () => api.get("/api/company/history").then((r) => r.data),
};

export const internshipApi = {
  research: (d: { target_role?: string; education_level?: string; college_tier?: string; available_from?: string; duration_months?: number }) =>
...
export const goalsApi = {
  ...
  create: (d: { title: string; target_date?: string }) => api.post("/api/goals", d).then((r) => r.data),
  update: (id: string, d: { is_completed?: boolean; title?: string }) =>
    api.patch(`/api/goals/${id}`, d).then((r) => r.data),
...
export const chatApi = {
  // Longer timeout: chat can be slowed by free-tier per-minute rate-limit retries.
  send: (d: { message: string; target_role?: string; history?: { role: string; content: string }[] }) =>
    api.post("/api/chat/message", d, { timeout: 120000 }).then((r) => r.data),
};
